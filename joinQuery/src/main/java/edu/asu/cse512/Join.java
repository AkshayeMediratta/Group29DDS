...
public class Join {
	...
	public static void main(String[] args) {
		...
		//String inputLocation1 = "hdfs://192.168.139.149:54310/harsh/JoinQueryInput3.csv";
		//String inputLocation2 = "hdfs://192.168.139.149:54310/harsh/JoinQueryInput2.csv";
		//String outputLocation = "hdfs://192.168.139.149:54310/harsh/JoinQueryResult.csv";
	...
	private static void spatialJoinQuery(String inputLocation1, String inputLocation2, String outputLocation,
			String inputType) {
		...
		if (inputType.equalsIgnoreCase("rectangle")) {
			...
			final JavaRDD<RectangleJoin> firstInputRDD = firstInputPoints.map(mapInputStringToRectRDD());

			// Map the query window to RDD object
			final JavaRDD<RectangleJoin> secondInputRDD = secondInputPoints.map(mapInputStringToRectRDD());

			// broadcast the second set of rectangles (query windows) to each of
			// the workers
			final Broadcast<List<RectangleJoin>> firstInput = sc.broadcast(firstInputRDD.collect());

			// map the id of second input to the multiple id’s of the first
			// input if they contain the
			// second rectangle
			joinQueryRDD = secondInputRDD.map(new Function<RectangleJoin, Tuple2<Integer, ArrayList<Integer>>>() {
				public Tuple2<Integer, ArrayList<Integer>> call(RectangleJoin rectangle) throws Exception {

					// Get the list of rectangles from the second RDD input.
					List<RectangleJoin> firstInputCollection = firstInput.value();

					ArrayList<Integer> firstInputIds = new ArrayList<Integer>();

					// Iterate the second input and check for the second set of
					// rectangle id’s
					// that hold the rectangle from first set obtained from the
					// mapped RDD
					for (RectangleJoin firstRects : firstInputCollection) {
						if (rectangle.isRectangleinsideQueryWindow(firstRects) || rectangle.isOverlap(firstRects)) {
							firstInputIds.add(firstRects.getRectangleId());
						}
					}

					// Create a new tuple of the mapped values and return back
					// the mapped transformation.
					Tuple2<Integer, ArrayList<Integer>> resultList = new Tuple2<Integer, ArrayList<Integer>>(rectangle
							.getRectangleId(), firstInputIds);
					return resultList;
				}
			});
			...
			final JavaRDD<PointJoin> firstInputRDD = firstInputPoints.map(mapInputStringToPointRDD());

			// broadcast the first input to each of the workers
			final Broadcast<List<PointJoin>> firstInput = sc.broadcast(firstInputRDD.collect());
			...
			joinQueryRDD = secondInputRDD.map(new Function<RectangleJoin, Tuple2<Integer, ArrayList<Integer>>>() {
				public Tuple2<Integer, ArrayList<Integer>> call(RectangleJoin rectangle) throws Exception {
					...
					// Get the list of rectangles from the second RDD input.
					List<PointJoin> firstInputCollection = firstInput.getValue();
					ArrayList<Integer> secondInputIds = new ArrayList<Integer>();

					// Iterate the first input and check for the second set of
					// rectangle id’s
					// that hold the points from first set obtained from the
					// mapped RDD
	...
	private static Function<String, RectangleJoin> mapInputStringToRectRDD() {
		return new Function<String, RectangleJoin>() {
			public RectangleJoin call(String inputString) {
				...
				PointJoin upperLeftPoint = new PointJoin(leftMostUpperXCoord, leftMostUpperYCoord);
				...
				PointJoin lowerRightPoint = new PointJoin(rightMostLowerXCoord, rightMostLowerYCoord);
				return new RectangleJoin(Integer.parseInt(points[0]), upperLeftPoint, lowerRightPoint);
			}
		};
	}
	...
	public static Function<String, PointJoin> mapInputStringToPointRDD() {
		return new Function<String, PointJoin>() {
			public PointJoin call(String inputString) {
				...
				return new PointJoin(Integer.parseInt(points[0]), Double.parseDouble(points[1]),
